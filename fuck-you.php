...
$greetings = 'СОСИ ХУЙ!';
$link = 'http://frendors.com/fuck-you.php';
$asshole_title = 'СОСИ ХУЙ!';

if(!empty($id)){
    $sql = "SELECT *  FROM `fuck_you` WHERE `id`={$id}";	
    ...
    $asshole = $query->fetchAll(PDO::FETCH_ASSOC);
        ...
        $asshole_name = $asshole[0]['text'];
        $asshole_title = 'СОСИ ХУЙ, ' . $asshole_name . '!';

        $greetings = 'СОСИ ХУЙ, <span style = "text-decoration:underline;">' . $asshole[0]['text'] . '</span>!';
        
        $link = 'http://frendors.com/fuck-you.php?id=' . $id; 
            
        }else {
            header('Location: https://frendors.com/fuck-you.php');
...
<meta property="og:description" content="Просто соси хуй"/>
<meta property="og:image" content="fuck-you.png">
<meta property="og:type" content="article"/>
<meta property="og:url" content= "//frendors.com/fuck-you.php" />
...
<script>
    ...
    function showLink(str){
     ...
     if (str.length == 0 || str.value == "" || str == null) {
        ...
        xmlhttp.onreadystatechange = function() {
            if (this.readyState == 4 && this.status == 200) {
                            ...
                            if(json_resp[0] === 'reserved'){ // Если уже существует
                                    ...
                                    document.getElementById("txtHint").innerHTML = "Уже сосет хуй";
                            
                                
                            }else{
                                 window.location.replace("https://frendors.com/fuck-you.php?id=" + json_resp[0]);
                            }
        ...
        formData.append('ASSHOLE',str.value.trim());
        
        xmlhttp.open("POST", "fuck-you-handler.php", true);
...
<div class="w3-card w3-round w3-white" style = "padding-top:10px;">
  ...
  src = "fuck-you.png" width = "170" height = "170" />
...
<h1 style = "font-weight:bold"><?php echo $greetings; ?></h1>

<div id = "form" class = "w3-panel w3-border w3-round w3-border-light-blue w3-padding" style = "margin-top:-10px;">
<label class="w3-text-deep-orange" style = "font-size:20px;">Кто сосет хуй следующий?</label><br />
<input type = "text" class = "w3-input w3-border w3-sand" maxlength = "40" id = "id" value = "" autocomplete="off" onkeypress = "showLinkEnter(event)" /><br />
<div id="txtHint" style = "margin-top:-20px;margin-bottom:1px;"></div>
<input type = "submit" class = "w3-button w3-block w3-large w3-theme" value = "OK" style = "margin:5px 0px" onclick = "showLink(document.getElementById('id'))" />
</div>
...
<div>
        ...
        pageImage: 'https://frendors.com/fuck-you.png'
        });
        </script>  
    ...
    $sql = "SELECT *  FROM `fuck_you` ORDER BY `date` DESC";	
    $query = $connection_handler->prepare($sql);
    $query->execute();
   ...
   for($i = 0; $i < count($assholes); $i++){
      $assholes_block .=  '<tr>
          ...
          <td>
              <a href = "fuck-you.php?id=' . $assholes[$i]['id']  . '">' . $assholes[$i]['text'] . '</a>
          </td>
          <td>
              <i  class = "fa fa-forward" style = "cursor:pointer" onclick = "window.open(\'https://yandex.ru/search/?text=' . $assholes[$i]['text'] . '\')"></i>
          </td>
          </tr>';
   }
...
<h2>Они сосут | <?php echo count($assholes); ?></h2>
