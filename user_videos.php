...
$sql = "SELECT *  FROM `user_videos` WHERE `user`='{$login}' ORDER BY `this_id` DESC";	
$query = $connection_handler->prepare($sql);
$query->execute();
...
<head>
<title>Видио | <?php echo $login; ?></title>
<meta charset = "utf-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name = "description" content = "Видио | <?php echo $login;  ?>" />
    <meta name="keywords" content = "<?php echo $login; ?>,зеленый слоник, бердянск,больничка, сергей пахомов, владимир епифанцев,александр маслаев,александр осмоловский,сладкий хлеб,азов,украина,видео" / >
    <meta name = "author" content = "<?php echo $login; ?>">

<meta property="og:title" content="Видио | <?php echo $login;  ?>"/>
...
<meta property="og:url" content= "//frendors.com/user_videos.php?zs=<?php echo $login; ?>" />
...
$whose_videos = "Видио " . count($videos) . $add_video_a;
    
}else{
   $whose_videos = "Видио | " . count($videos)  . " | " . $login; 
}

?>

<a href = "user.php?zs=<?php echo $login; ?>"> <<< <?php echo $login; ?></a>
<div id="vk_like"></div>
<script type="text/javascript">
    VK.Widgets.Like("vk_like", {type: "button"});
</script>
 <div class=fb-like id = fb_like_top data-href=https://frendors.com/user_videos.php?zs=<?php echo $login; ?> data-layout=button_count data-action=like data-size=small data-show-faces=true data-share=true></div>
<?php 
...
<h1 style = "color:blue;text-align:center;position:relative;top:20px;"><?php echo $whose_videos; ?></h1>
<p style = "color:green">Хорошие видео просто</p>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Добавляем видио</h4>
      </div>
      <div class="modal-body">
                <span>Список разрешенных ссылок:<span><ul>
...
function add_video(){
    ...
    if(!video_link_input.value.match(/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(youtube\.com\/watch\?v=)([a-zA-Z0-9]{1,256})/ig) && 
       ...
       !video_link_input.value.match(/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(ru.)?(redtube\.com\/)([0-9]{1,256})+/ig) &&
       !video_link_input.value.match(/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?([m\.]*)?(xvideos\.com\/video)([0-9]{1,8})([\/])([a-zA-Z0-9\-_]{1,256})/ig)
       ){
...
if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
  ...
  formData.append('link', video_link_input.value.trim());
   ...
   xmlhttp.open('POST', 'scripts/add_video_user_page.php', true);
...
if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
  ...
  formData.append('video_id', video_id);
   ...
   xmlhttp.open('POST', 'scripts/delete_user_video.php', true);
   xmlhttp.send(formData);

}

function add_video_from_other_page(obj,video_id){
...
if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
  ...
  formData.append('video_id', video_id);
   ...
   xmlhttp.open('POST', 'scripts/add_video_from_other_page.php', true);
   xmlhttp.send(formData);
...
function tell_to_bros_ahuenno(a_href,id){
...
xmlhttp.open('POST', 'scripts/tell_to_bros_ahuenno_user_video.php', true);
xmlhttp.send(formData);
...
function tell_to_bros_gavno(a_href,id){
...
xmlhttp.open('POST', 'scripts/tell_to_bros_gavno_user_video.php', true);
xmlhttp.send(formData);
...
<div id = "videos_container" class = "container-fluid">

<?php 

function youtube_name($hash){ // Извлекаем название у видоса с ютуба
...
// Видео с ютуба
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(youtube\.com\/watch\?v=)([a-zA-Z0-9\-_]{1,256})/is',$text) === 1){ 
        ...
        $multimedia_under_post_for_arr = "<iframe class = 'video_under_comment' src = '" . $outp_src . "' frameborder = '0' allowfullscreen></iframe>";
     ...
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?([m\.]*)?(xvideos\.com\/video)([0-9]{1,8})([\/])([a-zA-Z0-9\-_]{1,256})/is',$text) === 1){ 
        ...
        $multimedia_under_post_for_arr = '<iframe class = "video_under_comment" src=' . $outp_src . ' frameborder=0 allowfullscreen=allowfullscreen></iframe>';
         array_push($multimedia_under_post_arr,$multimedia_under_post_for_arr);
...
array_push($multimedia_under_post_arr,$multimedia_under_post_for_arr);
     ...
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?((rt\.)?pornhub\.com\/view_video\.php\?viewkey=)([a-zA-Z0-9\-_]{1,256})/is',$text) === 1){ 
        ...
        $hash = preg_replace('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?((rt\.)?pornhub\.com\/view_video\.php\?viewkey=)/is',"",$multimedia_src_arr[0]);
...
if(preg_match('/(^|[\n ])([\w]*?)(\/\/)?(vk\.com\/video_ext\.php\?oid=)([a-zA-Z0-9&-=])+/is',$text) === 1){ 
        ...
        $multimedia_under_post_for_arr = "<iframe class = 'video_under_comment' src = '" . $multimedia_src_arr[0] . "' frameborder = '0' allowfullscreen></iframe>";
        array_push($multimedia_under_post_arr,$multimedia_under_post_for_arr);
...
$video_name = "";
    ...
    for($i = 0; $i < count($videos); $i++){
                    ...
                    if($_SESSION['user'] === $login){
                        ...
                        $add_video_from_other_page_function = " | <a onclick = add_video_from_other_page(this," . $videos[$i]['this_id'] . ")>Добавить к себе</a>";
                    }
                    
                    if(!empty($_SESSION['user'])){

                    $tell_to_bros_ahuenno_function = " | <a onclick = tell_to_bros_ahuenno(this," . $videos[$i]['this_id'] . ")>на малафильню ахуенно</a>";
                    $tell_to_bros_gavno_function = " | <a onclick = tell_to_bros_gavno(this," . $videos[$i]['this_id'] . ")>на малафильню гавно</a>";

                    $ahuenno_video_function = " onclick = set_ahuenno_under_video(". $videos[$i]['this_id'] . ") ";
                    $gavno_video_function = " onclick = set_gavno_under_video(" . $videos[$i]['this_id'] . ")";
        ...
        // Видео с ютуба
         if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(youtube\.com\/watch\?v=)([a-zA-Z0-9\-_]{1,256})/is',$videos[$i]['link']) === 1){ 
            ...
            $video_name = youtube_name($hash);
              ...
              // Видео с xvideo
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?([m\.]*)?(xvideos\.com\/video)([0-9]{1,8})([\/])([a-zA-Z0-9\-_]{1,256})/is',$videos[$i]['link']) === 1){ 

        preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?([m\.]*)?(xvideos\.com\/video)([0-9]{1,8})([\/])([a-zA-Z0-9\-_]{1,256})/is',$videos[$i]['link'], $video_src_arr);  // Возвращает совпадение
        
        $outp_src = "";
    
        $hash = preg_replace('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?([m\.]*)?(xvideos\.com\/video)/is',"",$video_src_arr[0]);
        ...
        $outp_src = "https://www.xvideos.com/embedframe/" . $hash; 
         ...
         // Видео с редтуба
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(ru.)?(redtube\.com\/)([0-9]{1,256})/is',$videos[$i]['link']) === 1){ 

        preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(ru.)?(redtube\.com\/)([0-9]{1,256})/is',$videos[$i]['link'], $video_src_arr);  // Возвращает совпадение
  
        $hash = preg_replace('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?(ru.)?(redtube\.com\/?)/is',"",$video_src_arr[0]);
     ...
     // Видео с порнхаба
     if(preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?((rt\.)?pornhub\.com\/view_video\.php\?viewkey=)([a-zA-Z0-9\-_]{1,256})/is',$videos[$i]['link']) === 1){ 

        preg_match('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?((rt\.)?pornhub\.com\/view_video\.php\?viewkey=)([a-zA-Z0-9\-_]{1,256})/is',$videos[$i]['link'], $video_src_arr);  // Возвращает совпадение 
        
        $outp_src = "";
         $multimedia_under_post_for_arr = "";
      
        $hash = preg_replace('/(^|[\n ])([\w]*?)(https?:\/\/)?(www.)?((rt\.)?pornhub\.com\/view_video\.php\?viewkey=)/is',"",$video_src_arr[0]);
   ...
   $current_time = time();
       ...
       // ЛАЙКИ
     $sql = "SELECT COUNT(*) AS `ahuenno_count` FROM `user_videos_likes` WHERE `video_id`={$videos[$i]['this_id']}";	
     $query = $connection_handler->prepare($sql);
     $query->execute();
    ...
    // ДИЗЛАЙКИ
     $sql = "SELECT COUNT(*) AS `gavno_count` FROM `user_videos_dislikes` WHERE `video_id`={$videos[$i]['this_id']}";	
     $query = $connection_handler->prepare($sql);
     $query->execute();
          ...
          $send_comment_form = "";
                    ...
                    if(!empty($_SESSION['user'])){
                        $send_comment_form = "<div class = send_comment_form>
                                            <textarea style='width:100%' rows=1 id = 'comment_textarea_" . $videos[$i]['this_id'] . "' onkeypress = send_comment_enter(event," . $videos[$i]['this_id'] . ") oninput = this.rows=3;this.placeholder='' maxlength = '1000' placeholder = 'malafeel'></textarea>
                                            <button type = 'button' class='btn btn-success btn-sm' id = 'send_comment_button_" . $videos[$i]['this_id'] . "' onclick = send_comment(". $videos[$i]['this_id'] . ")>Малафить</button>
                                        </div>" ;
            }

    // КОММЕНТЫ
    $sql = "SELECT *  FROM `user_videos_comments` WHERE `video_id`={$videos[$i]['this_id']} ORDER BY `this_id` ASC";	
    $query = $connection_handler->prepare($sql);
    $query->execute();
                        ...
                        <div class = 'video_info_container col-md-2'><a href = " . $videos[$i]['link']  . " target = '_blank'>" . $video_name . "</a>
                           ...
                           '<div class = "functions_under_video_container">' .  $delete_video_function . $add_video_from_other_page_function . " | <a href = ". $videos[$i]['link']  . " target = _blank>Ссылка</a></div>
                             <div class = ahuenno_gavno_under_video_container>" .
                              $tell_to_bros_ahuenno_function .
                            "<span class = ahuenno_video_span " . $ahuenno_video_function . "><img src = imgs/like.png class = img_ahuenno />Ахуенно | <span class = ahuenno_count_span id = ahuenno_count_span_" . $videos[$i]['this_id']  . " >" . $ahuenno_count . "</span></span>" . 
                             $tell_to_bros_gavno_function .  "<span class = gavno_video_span " . $gavno_video_function . "><img src=imgs/dislike.png class = img_gavno />Гавно! | <span class = gavno_count_span id = gavno_count_span_" . $videos[$i]['this_id']  . " >" . $gavno_count . "</span></span>
                             </div> 
...
function send_comment(video_id){
    ...
    var send_comment_button = document.getElementById("send_comment_button_" + video_id);
...
xmlhttp.open('POST', 'scripts/user_videos_comments.php', true);
xmlhttp.send(formData);
...
xmlhttp.open('POST', 'scripts/delete_comment_under_user_video.php', true);
xmlhttp.send(formData);
...
xmlhttp.open('POST', 'scripts/set_like_user_video.php', true);
xmlhttp.send(formData);
    
}

function set_gavno_under_video(video_id){
    
    var gavno_count_span = document.getElementById("gavno_count_span_" + video_id);
...
xmlhttp.open('POST', 'scripts/set_dislike_user_video.php', true);
xmlhttp.send(formData);
