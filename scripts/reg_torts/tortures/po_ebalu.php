...
$sql = "SELECT * FROM `po_ebalu` WHERE `clicker`='" . $_SESSION['user'] . "' AND `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();
$po_ebalu = $query->fetchAll(PDO::FETCH_ASSOC);      

if(empty($po_ebalu)){ // Пользователь папиросы не давал
 
/* Заносим папирос в БД */
$sql = "INSERT INTO `po_ebalu`(`clicker`,`page_id`) VALUES ('{$_SESSION['user']}','{$page_id}')";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();    

}else { // Пользователь папирос давал
   /* Удаляем папирос  из базы */
$sql = "DELETE FROM `po_ebalu` WHERE `clicker`='{$_SESSION['user']}' AND `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute(); 
}

/* Отображаем все папиросы */    
$sql = "SELECT COUNT(*) AS `po_ebalu_count` FROM `po_ebalu` WHERE `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();
$po_ebalu_count = $query->fetchAll(PDO::FETCH_ASSOC);      
$po_ebalu_count = $po_ebalu_count[0]['po_ebalu_count'];
echo $po_ebalu_count; 
