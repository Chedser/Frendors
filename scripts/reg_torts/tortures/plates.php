...
$sql = "SELECT * FROM `plates` WHERE `clicker`='" . $_SESSION['user'] . "' AND `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();
$plates = $query->fetchAll(PDO::FETCH_ASSOC);      

if(empty($plates)){ // Пользователь папиросы не давал
 
/* Заносим папирос в БД */
$sql = "INSERT INTO `plates`(`clicker`,`page_id`) VALUES ('{$_SESSION['user']}','{$page_id}')";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();    

}else { // Пользователь папирос давал
   /* Удаляем папирос  из базы */
$sql = "DELETE FROM `plates` WHERE `clicker`='{$_SESSION['user']}' AND `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute(); 
}

/* Отображаем все папиросы */    
$sql = "SELECT COUNT(*) AS `plates_count` FROM `plates` WHERE `page_id`='{$page_id}'";	
$query = $connection_handler_tortures->prepare($sql); //Подготавливаем запрос
$query->execute();
$plates_count = $query->fetchAll(PDO::FETCH_ASSOC);      
$plates_count = $plates_count[0]['plates_count'];
echo $plates_count; 
