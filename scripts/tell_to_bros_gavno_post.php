...
if(!empty($post_id) && !empty($_SESSION['user'])){
    ...
    $tell_to_bros_ahuenno_post = $query->fetchAll(PDO::FETCH_ASSOC);
            ...
            // ВСТАВЛЯЕМ В ТАБЛИЦУ РАССКАЗАННЫХ ПОСТОВ
                $sql = "INSERT INTO `tell_to_bros_gavno_post`(`teller`,`post_id`) VALUES('{$_SESSION['user']}',{$post_id})"; 
                $query = $connection_handler->prepare($sql);
                $query->execute();
            ...
            // ВСТАВЛЯЕМ В ТАБЛИЦУ ПОСТОВ
                $sql = "INSERT INTO `wall_user_posts`(`page_id`,`nickname`,`message`,`post_sender_gavno`,`type`) VALUES('{$_SESSION['user']}','{$_SESSION['user']}','{$post_text}','{$post_sender}','telled_gavno')"; 
                $query = $connection_handler->prepare($sql);
                $query->execute(); 
...
$post_sender = $post_info[0]['nickname']; // Тот кто оставил пост
$post_text = $post_info[0]['message']; // Тот кто оставил пост
$page_id = $post_info[0]['page_id']; // Страница поста
...
/*******************************************************************************************************/
        ...
        if($post_sender !== $_SESSION['user']){ // Пост отправил не сессионный. Оповещаем отправителя 
            ...
            $sql = "SELECT * FROM `notice` WHERE `post_id`={$post_id} AND `is_gavno_teller`=1";
            $query = $connection_handler->prepare($sql);
            $query->execute();  
            $gavno_teller_notice = $query->fetchAll(PDO::FETCH_ASSOC);
            
            if(empty($ganvo_teller_notice)){
           ...
           $whose_page = ""; 
            ...
            if($post_sender !== $page_id){
             ...
             $sql = "INSERT INTO `notice`(`noticer`,`noticed`,`description`,`type`,`post_sender`,`page`,`post_id`,`post_text`,`is_gavno_teller`) VALUE ('{$_SESSION['user']}','{$post_sender}','{$whose_page}','tell_gavno_post','{$post_sender}','{$page_id}',{$post_id},'{$post_text}',1)";
            $query = $connection_handler->prepare($sql);
            $query->execute();  
            
             // ОПОВЕЩАЕМ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ 
            $sql = "INSERT INTO `common_notice`(`noticer`,`type`,`page`,`post_sender`,`post_id`,`post_text`) VALUES ('" . $_SESSION['user'] . "','tell_to_bros_gavno_post','{$whose_page}','{$post_sender}',{$post_id},'{$post_text}')";
            $query = $connection_handler->prepare($sql);
            $query->execute();
