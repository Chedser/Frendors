...
if(empty($dislikes_user) && empty($likes_user)){ // Пользователь лайк не ставил
    /* Заносим лайк в БД */
$sql = "INSERT INTO `common_discussions_dislikes`(`disliker`,`story_id`) VALUES ('{$_SESSION['user']}',{$story_id})";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
...
$sql = "SELECT COUNT(*) AS `dislikes_count` FROM `common_discussions_dislikes` WHERE `story_id`={$story_id} AND `post_id`={$post_id}";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
