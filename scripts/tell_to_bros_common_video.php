...
// ВСТАВЛЯЕМ КАРТИНКУ В ТАБЛИЦУ ПОСТОВ
    $sql = "INSERT INTO `wall_user_posts`(`page_id`,`nickname`,`post_sender_ahuenno`,`message`,`link`,`multimedia_id`,`whose_multimedia`,`type`) VALUES('{$_SESSION['user']}','{$_SESSION['user']}','{$_SESSION['user']}','{$link}','{$link}',{$video_id},'{$whose_video}','telled_ahuenno_common_video')"; 
    $query = $connection_handler->prepare($sql);
    if($query->execute()){
        $outp .= '"status":"true"}';
    }else{
...
echo $outp;
    ...
    if($whose_video !== $_SESSION['user']){ // Картинку отправил не сессионный. Оповещаем отправителя 
            ...
            if(empty($ahuenno_teller_notice)){
                             ...
                             $sql = "INSERT INTO `notice`(`noticer`,`noticed`,`multimedia_id`,`link`,`type`,`is_ahuenno_teller`) VALUE ('{$_SESSION['user']}','{$whose_video}',{$video_id},'{$link}','telled_ahuenno_common_video',1)";
                            $query = $connection_handler->prepare($sql);
                            $query->execute();  
                            
            }
    // ОПОВЕЩАЕМ ВСЕХ ПОЛЬЗОВАТЕЛЕЙ 
 $sql = "INSERT INTO `common_notice`(`noticer`,`whose_multimedia`,`multimedia_id`,`link`,`type`) VALUE ('{$_SESSION['user']}','{$whose_video}',{$video_id},'{$link}','telled_ahuenno_common_video')";
 $query = $connection_handler->prepare($sql);
 $query->execute(); 
