...
if($user_dislikes_count == 0){ // Пользователь лайк не ставил
...
$user_likes_count = (int)$user_likes_count[0]['user_likes_count'];
    ...
    /* Заносим лайк в БД */
$sql = "INSERT INTO `primary_album_dislikes`(`image_id`,`disliker`) VALUES ({$image_id},'{$_SESSION['user']}')";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
}
...
$sql = "DELETE FROM `primary_album_dislikes` WHERE  `image_id`={$image_id} AND `disliker`='{$_SESSION['user']}'";	
$query = $connection_handler->prepare($sql); 
$query->execute();
