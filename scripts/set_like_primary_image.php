...
$user_dislikes_count = (int)$user_dislikes_count[0]['user_dislikes_count'];
    ...
    /* Заносим лайк в БД */
$sql = "INSERT INTO `primary_album_likes`(`image_id`,`liker`) VALUES ({$image_id},'{$_SESSION['user']}')";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
}
...
$sql = "DELETE FROM `primary_album_likes` WHERE  `image_id`={$image_id} AND `liker`='{$_SESSION['user']}'";	
$query = $connection_handler->prepare($sql); 
$query->execute();
