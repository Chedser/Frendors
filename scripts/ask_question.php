...
// ПРОВЕРЯЕМ СУЩЕСТВУЕТ ЛИ ПОМОЩНИК В ГЛАВНОЙ БАЗЕ
...
$supporter = $query->fetchAll(PDO::FETCH_ASSOC);
    ...
    if(!empty($user) && !empty($supporter)){
     
             // ПРОВЕРЯЕМ ЕСТЬ ЛИ ВОПРОСЫ ОТ ПОЛЬЗОВАТЕЛЯ  
        $sql = "SELECT * FROM `support_questions` WHERE `user`='{$_SESSION['user']}' ORDER BY `this_id` DESC";	
        $query = $connection_handler->prepare($sql); 
        $query->execute();
        $user_questions = $query->fetchAll(PDO::FETCH_ASSOC);
            ...
            if(empty($user_questions)){
                    ...
                    $sql = "INSERT INTO `support_questions`(`supporter`,`user`,`question_header`,`question_text`,`time_unix`) 
                    VALUES ('" . $supporter[0]['user'] . "','" . $_SESSION['user'] . "','" . $header . "','" . $text . "'," . $time_unix . ")";
                    $query = $connection_handler->prepare($sql);
                    if($query->execute()){
