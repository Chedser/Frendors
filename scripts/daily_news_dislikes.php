...
if(empty($likes) && empty($indifferents)){
$sql = "INSERT INTO `daily_news_dislikes`(`disliker`,`news_id`) VALUES ('{$_SESSION['user']}',{$daily_news_id})";	
$query = $connection_handler->prepare($sql); 
$query->execute();
}
...
$sql = "DELETE FROM `daily_news_dislikes` WHERE `disliker`='{$_SESSION['user']}' AND `news_id`={$daily_news_id}";	
$query = $connection_handler->prepare($sql); 
$query->execute();
...
$sql = "SELECT COUNT(*) AS `daily_news_dislikes_count` FROM `daily_news_dislikes` WHERE `news_id`='{$daily_news_id}'";	
$query = $connection_handler->prepare($sql); 
$query->execute();
$daily_news_dislikes_count_all = $query->fetchAll(PDO::FETCH_ASSOC);      
$daily_news_dislikes_count_all = $daily_news_dislikes_count_all[0]['daily_news_dislikes_count'];

echo $daily_news_dislikes_count_all;
