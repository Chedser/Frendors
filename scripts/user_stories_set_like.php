...
$story_id = (int) htmlspecialchars($_POST['story_id']);
...
if(empty($likes_user) && empty($dislikes_user)){ // Пользователь лайк не ставил
    /* Заносим лайк в БД */
$sql = "INSERT INTO `user_stories_likes`(`liker`,`story_id`) VALUES ('{$_SESSION['user']}',{$story_id})";	
...
$sql = "SELECT COUNT(*) AS `likes_count` FROM `user_stories_likes` WHERE `story_id`={$story_id}";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
