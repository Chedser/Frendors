...
if(empty($comment_likes)){ // Пользователь лайк не ставил
    ...
    if(empty($comment_dislikes)){
        /* Заносим лайк в БД */
    $sql = "INSERT INTO `comments_likes`(`liker`,`page`,`post_id`,`comment_id`) VALUES ('{$_SESSION['user']}','{$page}',{$post_id},{$comment_id})";	
    $query = $connection_handler->prepare($sql); //Подготавливаем запрос
    $query->execute();
}
...
$sql = "DELETE FROM `comments_likes` WHERE  `post_id`={$post_id} AND `comment_id`={$comment_id}";	
...
$sql = "SELECT COUNT(*) AS `comment_likes_count` FROM `comments_likes` WHERE `post_id`={$post_id} AND `comment_id`={$comment_id}";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();
$comment_likes_count = $query->fetchAll(PDO::FETCH_ASSOC);      
$comment_likes_count = $comment_likes_count[0]['comment_likes_count'];

echo $comment_likes_count;
