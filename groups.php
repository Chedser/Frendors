...
<html>
<head>
    <title>Гауптвахты</title>
    <meta charset = "utf-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name = "description" content = "Гапутвахты <?php echo $login; ?>" />
    <meta name="keywords" content = "гауптвахты,зеленый слоник, бердянск,больничка, сергей пахомов, владимир епифанцев,александр маслаев,александр осмоловский,сладкий хлеб,азов,украина,путин,политика" / >
    <meta name = "author" content = "<?php echo $login; ?>">

<meta property="og:title" content="Гауптвахты <?php echo $login; ?>"/>
<meta property="og:description" content="Гауптвахты братка"/>
<meta property="og:image" content="users/<?php echo $avatar_for_vk; ?>">
<meta property="og:type" content="article"/>
<meta property="og:url" content= "//frendors.com/groups.php?zs=<?php echo $login; ?>" />
...
if(session_isset() && $login === $_SESSION['user']){
    $create_group_button =  "<button type = 'button' id = 'create_group_button' class = 'btn btn-success btn-lg' data-toggle='modal' data-target='#create_group_box'
                 onclick = 'show_create_group_box()' title = 'создать группу' >
                Построить гауптвахту
                </button>";
}
...
$sql = "SELECT *  FROM `group_members` WHERE `member`='{$login}' ORDER BY `this_id` DESC";	
$query = $connection_handler->prepare($sql);
$query->execute();
...
<div id = "center_container" class = "col-md-10">
    ...
    <main id = "center">
        ...
        <h3 style="color:blue;text-decoration:underline;display:inline-block;">Гауптвахты <?php echo $whose_groups .  $groups_count; ?></h3>
                <?php echo $create_group_button; ?>
            </div>
        <hr />    
        <!-- Модальное окно для личного сообщения -->
  <div class="modal fade" id="create_group_box" role="dialog">
    <div class="modal-dialog">
      ...
      <div class="modal-content">
        <div class="modal-header">
          ...
          <h4 class="modal-title">Построить гауптвахту</h4>
        </div>
        
        <div class="modal-body">
            <span>Название</span>
            <input type = "text" id = "name_group_input" maxlength = "100" style="width:100%;display:block;margin-bottom:5px;" autofocus = "autofocus" required = "required" /> 
        <span id = "error_span" style = "color:red;display:none;">Недопустимое название нахуй!<br /></span>
        <button type = "button" class = "btn btn-primary btn-md" onclick = create_group(this)>Ахуенно</button>
...
var create_group_box = document.getElementById('create_group_box');
function show_create_group_box(){
    create_group_box.style.display = "block";    
}

function close_create_group_box(){
create_group_box.style.display = "none";   
}

/*****************************************/

function create_group(button){

var name_group_input = document.getElementById("name_group_input");
var error_span = document.getElementById("error_span");
var name_group_length = name_group_input.value.length;
var response = "";

if(name_group_length >=2 && name_group_length <=100){
    ...
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
      response = JSON.parse(xmlhttp.responseText);
           switch(response.server_response){
          case 'empty': error_span.innerHTML = "Недопустимое название нахуй!";break;
          case 'unknoun_error': error_span.innerHTML = "Неизвестная ошибка";break;
          case 'success':  
              error_span.style.display = "inline";
              error_span.innerHTML = "Гауптвахта построена нахуй!";
              name_group_input.value= "";
...
formData.append('name', name_group_input.value);

xmlhttp.open("POST", "handlers/create_group.php", true);
...
for($i = 0; $i < count($groups);$i++){
    ...
    $name = $groups[$i]['name'];
        ...
        $group_avatar = $group_info[0]['avatar'];
        ...
        $query = $connection_handler->prepare($sql);
        $query->execute();
        $group_members_count = count($query->fetchAll(PDO::FETCH_ASSOC));
        
    
    echo '<div class = group_info_container style = height:100px onclick = window.location.replace("group.php?id=' . $group_id .'")>
                <div style = float:left;margin-right:5px;><img src = ' . $group_avatar . ' width = 100px height = 100px /></div>
                <h3 style = color:blue;text-decoration:underline;display:block>' . $group_name . '</h3>
                <span class = "group_status_span" >Сидят тута: ' .  $group_members_count . '</span>
            </div>';
