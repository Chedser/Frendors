...
echo "Пустое подтверждение пароля";
exit();
...
$dir = 'zs' . $connection_handler->lastInsertId() . '/'; // Папка пользователя 
...
$sql = "INSERT INTO `additional_info`(`nickname`,`user_id`) VALUES ('{$nick}',{$connection_handler->lastInsertId()})";	
$query = $connection_handler->prepare($sql); //Подготавливаем запрос
$query->execute();

 $data_array = array($nick, 'Я теперь долбоеб!', $nick);
    $sql = "INSERT INTO wall_user_posts(nickname,message,page_id) VALUES(?,?,?) "; 
    $query = $connection_handler->prepare($sql);
    $query->execute($data_array); 
session_start();
$_SESSION['user'] = $nick;
 header('Location: ../user.php?zs=' . $nick . "&ls=1"); // Редиректим на страницу пользователя
                      } // Пароли совпадают 
