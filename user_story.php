...
$sql = "SELECT * FROM `user_stories` WHERE `this_id`={$id}";	
...
$sql = "SELECT COUNT(*) AS `likes_count` FROM `user_stories_likes` WHERE `story_id`={$id}";	
$query = $connection_handler->prepare($sql); 
$query->execute();
$likes_count = $query->fetchAll(PDO::FETCH_ASSOC);
$likes_count = $likes_count[0]['likes_count']; 

$sql = "SELECT COUNT(*) AS `dislikes_count` FROM `user_stories_dislikes` WHERE `story_id`={$id}";	
$query = $connection_handler->prepare($sql); 
$query->execute();
...
$sql = "SELECT * FROM `user_stories_likes` WHERE `story_id`={$id} AND `liker`='{$_SESSION['user']}'";	
$query = $connection_handler->prepare($sql); 
$query->execute();
$likes = $query->fetchAll(PDO::FETCH_ASSOC);

$sql = "SELECT * FROM `user_stories_dislikes` WHERE `story_id`={$id} AND `disliker`='{$_SESSION['user']}'";	
$query = $connection_handler->prepare($sql); 
$query->execute();
...
$sql = "SELECT * FROM `user_stories_posts` INNER JOIN `additional_info` 
ON user_stories_posts.author=additional_info.nickname WHERE `story_id`={$id} 
ORDER BY user_stories_posts.this_id DESC";	
$query = $connection_handler->prepare($sql); 
$query->execute();
...
<meta property="og:url" content= "https://frendors.com/user_story.php?id=<?php echo $id; ?>" />
...
xmlhttp.open("POST", "scripts/user_stories_set_like.php", true);
...
xmlhttp.open("POST", "scripts/user_stories_set_dislike.php", true);
xmlhttp.send(formData);
...
xmlhttp.open("POST", "scripts/user_stories_posts.php", true);
xmlhttp.send(formData);    
...
<h6 class = "w3-left"><a href="user_stories.php?zs=<?php echo $author; ?>">Истории</a></h6>
<?php echo $vk; ?>
