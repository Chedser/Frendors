...
if(session_isset()){
  ...
  <a href="notices.php"><img class="topnav_img" src="imgs/header/opoves.png" title="Диндон">
   ...
   <a href="groups.php?zs=' . $_SESSION['user'] . '" class = "w3-hide-small"><img class="topnav_img" src="imgs/header/handcuffs.png" title="Группы"></a>
      ...
      <div class="w3-card w3-round w3-white block">
            <div class="w3-container">
                 <div id = "nick_pseudonick_container" style = "borrder:1px solid black">
                     <h4 class="w3-center nick" style = "text-decoration:underline;word-wrap:break-word"><?php echo $nickname; ?></h4>
...
if(empty($requests) && !empty($friend)){ // В друзьях. Пользователь добавил. Очистил строку запроса
 ...
 " onclick=document.getElementById('delete_friend_modal').style.display='block'" . " >Пошел нахуй!</button>
  <div id = 'deleted_friend_message' class = 'w3-panel w3-pale-green' style = 'display:none;'>Гавно!</div>";
     ...
     $delete_friend_vbratki_button = "<button id = 'vbratki_button' onclick = friend_request(this,{$user_id}) class = 'w3-button w3-block w3-theme' >В долбоебы</button>
...
$rating_button = '<button type = "button"  class = "w3-button w3-block w3-theme" id = "give_strap_button" ' . $give_coloner_pogons . ' 
...
$rating_button = "";    
 ...
 $doebat_button = "<button id = doebat_button class = 'w3-button w3-block w3-theme' onclick=document.getElementById('doebat_modal').style.display='block'>Доебать</button>";
 $mark_as_pidor_button = '<button id = "mark_as_pidor_button" class = "w3-button w3-block w3-theme" 
...
$buttons_container = '<div class="w3-card w3-round w3-white w3-padding-16" id = "buttons_panel" style = "margin-bottom:10px">
    ...
    echo '<div class="w3-card w3-round w3-white w3-padding-16" id = "buttons_panel" style = "margin-bottom:10px">
                <div class="w3-container">
                    <button type = "button"  class = "w3-button w3-block w3-theme" id = "groups_button"  title = "Сидит тута" onclick = window.location.replace("groups.php?zs=' . $login . '")>
                     Группы
                    </button></div></div>'; 
...
if(!session_isset()){  
    echo            ' <div class="w3-card w3-round w3-white w3-center" style = "padding-bottom:5px;margin-bottom:10px">
                    ...
                    <div class="fb-login-button" data-size="medium" style = "margin-top:5px;"
                            ...
                            data-default-audience="everyone" data-onlogin="checkLoginState()">
                    </div>
...
echo "<div class='w3-card w3-round w3-white w3-padding' style = 'margin-top:-15px;margin-bottom:10px'>
            <div class='w3-container'>
            <button type = 'button' class = 'w3-button w3-block w3-theme'" . 
...
for($i = 0; $i < count($friends_to); $i++){
    $friends_container .= "<a href = 'user.php?zs={$friends_to[$i]['friend2']}' title = '{$friends_to[$i]['friend2']}'><img src = 'users/{$friends_to[$i]['avatar']}'  class = 'bro_img' /></a>";
       if($i == 9){break;}
}
...
/*-*****************************************************************************/
        ...
        $avatar = $avatar[0]['avatar'];
           $common_friends_from_outp .= "<a href = 'user.php?zs={$common_friends_from[$i]}' title = '{$common_friends_from[$i]}'><img src = 'users/{$avatar}'  class = 'bro_img' /></a>"; 
        }
        ...
        $avatar = $query->fetchAll(PDO::FETCH_ASSOC);
        $avatar = $avatar[0]['avatar'];
           $common_friends_to_outp .= "<a href = 'user.php?zs={$common_friends_to[$i]}' title = '{$common_friends_to[$i]}'>
...
if(session_isset()){
    ...
    if(($_SESSION['user'] !== 'Frendors' && $login !== 'Frendors') || $_SESSION['user'] === 'Frendors'){
    $post_textarea = 
        ...
        <div class="w3-col m12">
          <div class="w3-card w3-round w3-white">
            ...
            <textarea id = "user_wall_message" placeholder = "пиши" class="w3-border w3-padding" 
                    ...
                    <button type="button" class="w3-button w3-block w3-theme" onclick = sendPost()><i class="fa fa-pencil"></i>Ахуенно</button> 
            </div>    
...
" INNER JOIN additional_info ON wall_user_posts.nickname=additional_info.nickname ORDER BY wall_user_posts.this_id DESC LIMIT 50";
    $query = $connection_handler->prepare($sql);
    $query->execute();
...
if($posts_count_on_wall > 0){
 for($i = 0; $i < $posts_count_on_wall; $i++ ){ // ОТОБРАЖАЕМ ВСЕ СООБЩЕНИЯ
    ...
    case 'telled_ahuenno_primary_image':
                                        ...
                                        <div class = 'w3-panel w3-pale-green' style = margin-bottom:-1px>
...
<div class="w3-card w3-white w3-round w3-margin"><br>
    ...
    <div class = "post_buttons_container">
              <button type="button" class="w3-button w3-theme-d1 w3-margin-bottom" ' . $tell_to_bros_ahuenno_function  . '>
              <i class="fa fa-thumbs-up"></i>Ахуенно <i class = "likers_count" id = "likers_count_' . $posts[$i]['post_id'] . '">' . $likers_count . '</i></button> 
             <button type="button" class="w3-button w3-theme-d2 w3-margin-bottom"' . $tell_to_bros_gavno_function  . '>
...
$all_user_avatar = $all_user_avatar[0]['avatar'];
    ...
    if($counter10 > 0){
       echo ' <div class="w3-card w3-round w3-white w3-center" style = "margin-top:-10px">
        <div class="w3-container">' .
                  ...
                  if($tmp_id_q != null && !in_array($ichoose,$choosed_ids)){
                     ...
                     $sql = "SELECT * FROM `additional_info` WHERE `user_id`={$tmp_id}"; 
                        ...
                        $avatar = $avatar[0]['avatar'];
                         ...
                         $users_container .= "<div style = 'display:inline-block'><a href = 'user.php?zs=" . $tmp_id_q[0]['nickname'] . "' title = '" .  $tmp_id_q[0]['nickname'] . "'>
